...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
	<head>
		...
		<title>Forensics | <?php echo $Translation['admin area']; ?><?php echo html_attr(isset($GLOBALS['page_title']) ? " | {$GLOBALS['page_title']}" : ''); ?></title>
	...
	<div class="container theme-superhero theme-compact">
		...
		<nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
			<div class="navbar-header">
				...
				<a style="background-color:blueviolet; border-radius:20px; " class="navbar-brand" href="pageHome.php"><span class="text-info"><i class="glyphicon glyphicon-cog"></i> <?php echo $Translation['admin area']; ?></span></a>
			</div>

			<div class="collapse navbar-collapse navbar-ex1-collapse"style="background-color:black; border-bottom-right-radius:20px;">
				<ul class="nav navbar-nav" style="background-color:grey; border-radius:20px;">
					<li class="dropdown" style="background-color:purple; border-radius:20px;">
					...
					<li class="dropdown" style="background-color:green; border-radius:20px;">
					...
					<li class="dropdown" >
						...
						<ul class="dropdown-menu">
							...
							<li style="background-color:red; border-bottom-right-radius:20px;"><a href="pageRebuildFields.php"><i class="glyphicon menu-item-icon text-info glyphicon-refresh"></i> <?php echo  $Translation['rebuild fields']; ?></a></li>
						...
						<li class="dropdown">
							...
							<ul class="dropdown-menu">
								<?php foreach($plugins as $plugin){ ?>
									...
									?>
									<li style="border-radius:20px;"><a target="_blank" href="<?php echo $plugin['admin_path']; ?>"><?php echo $plugin_icon . $plugin['title']; ?></a></li>
								<?php } ?>
							</ul>
						</li>
					<?php } ?>
				...
				<div class="navbar-right" >
					<a style="border-radius:20px;" href="<?php echo PREPEND_PATH; ?>index.php" class="btn btn-success navbar-btn"><?php echo $Translation["user's area"] ; ?></a>
					<a style="border-radius:20px;" href="<?php echo PREPEND_PATH; ?>index.php?signOut=1" class="btn btn-warning navbar-btn"><i class="glyphicon glyphicon-log-out"></i> <?php echo $Translation["sign out"] ; ?></a>
				</div>
			</div>
		</nav>
		<script>
...
<?php
	if(!strstr($_SERVER['PHP_SELF'], 'pageSettings.php') && password_match('admin', $adminConfig['adminPassword'])) {
		$noSignup = true;
		?>
		<div style="border-radius:20px;" class="alert alert-danger">
